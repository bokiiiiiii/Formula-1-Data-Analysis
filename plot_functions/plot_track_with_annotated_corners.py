...
def annotate_corners(ax, circuit_info, rotated_track, track_angle):
    ...
    for _, corner in circuit_info.corners.iterrows():
        ...
        ax.scatter(text_x, text_y, color="grey", s=140, label="Corners" if corner["Number"] == 1 else "")
        ax.plot([track_x, text_x], [track_y, text_y], color="grey")
        ax.text(
            text_x, text_y, txt, va="center_baseline", ha="center",
...
def add_scale(ax, x_max, y_min):
    ...
    ax.plot([scale_pos_x, scale_pos_x + scale_length], [scale_pos_y, scale_pos_y], color="white", lw=1.7)
    ax.plot([scale_pos_x, scale_pos_x], [scale_pos_y, scale_pos_y + 100], color="white", lw=1.7)
    ax.plot([scale_pos_x + scale_length, scale_pos_x + scale_length], [scale_pos_y, scale_pos_y + 100], color="white", lw=1.7)
    ax.text(scale_pos_x + scale_length / 2, scale_pos_y + 250, scale_text, ha="center", fontsize=10)
...
def plot_track_with_annotated_corners(year: int, event_name: str, session_name: str, race, post: bool) -> dict:
    race.load()
    circuit_name, locality, country, location = get_circuit_info_by_country(year, event_name)
    ...
    plt.figtext(
        0.5, 0.935, subtitle, ha="center", fontsize=14,
